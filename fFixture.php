...
class fFixture
{
    ...
    */
     ...
     */
    public function build($traverse = TRUE)
    {
        try {

            // Build queue

            $build_queue = $this->buildQueue(NULL, $traverse);
            ...
            foreach ($build_queue as $fixture_name) {
                $this->buildRecords($completed_fixtures, $fixture_name, $traverse);
            }
     ...
     */
    private function buildQueue($fixture_name = NULL, $traverse = TRUE)
    {
        $queue = array();
        ...
        if (is_null($fixture_name)) {
            ...
            foreach ($this->fixture_sources as $fixture_name => $fixture_source) {
                if ($this->isWhiteListed($fixture_name)) {
                    $queue = array_merge($queue, $this->buildQueue($fixture_name, $traverse));
                }
            }
